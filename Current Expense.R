...
ce.lower <- data.frame(NULL)
ce.higher <- data.frame(NULL)
for (i in 1:length(da)){
  for (j in levels(g[,1])){
    ...
    rdd.higher <- tryCatch(rdrobust(da[,i], df$pctfor, c = 0.5, h = 1.05*rdd$bws[1,],
                                    ...
                                    bwselect = j, all = 'iii'),
                           error = function(e) paste('na'))
    ce.lower[i, 1] <- names(da)[i]
    ce.higher[i, 1] <- names(da)[i]
    ce.lower[i, j] <- ifelse(is.atomic(rdd.lower) == TRUE, 'na', 
                             round(rdd.lower$pv[2], 3))
    ce.higher[i, j] <- ifelse(is.atomic(rdd.higher) == TRUE, 'na',
...
da <- cbind(da, de)
da <- da[,order(colnames(da))]
ce.lower.1995 <- data.frame(NULL)
ce.higher.1995 <- data.frame(NULL)
for (i in 1:length(da)){
  for (j in levels(g[,1])){
    rdd <- tryCatch(rdrobust(da[,i], df$pctfor, c = 0.5, covs = covs, covs_drop = T,
                             bwselect = j, all = 'iii'), 
    ...
    rdd.higher <- tryCatch(rdrobust(da[,i], df$pctfor, c = 0.5, h = 1.05*rdd$bws[1,],
                                    ...
                                    bwselect = j, all = 'iii'),
                           error = function(e) paste('na'))
    ce.lower.1995[i, 1] <- names(da)[i]
    ce.higher.1995[i, 1] <- names(da)[i]
    ce.lower.1995[i, j] <- ifelse(is.atomic(rdd.lower) == TRUE, 'na', 
                                  round(rdd.lower$pv[2], 3))
    ce.higher.1995[i, j] <- ifelse(is.atomic(rdd.higher) == TRUE, 'na',
                                   round(rdd.higher$pv[2], 3))
  }
}
summary(rdd)
...
da <- cbind(da, de)
da <- da[,order(colnames(da))]
ce.lower.2011 <- data.frame(NULL)
ce.higher.2011 <- data.frame(NULL)
for (i in 1:length(da)){
  for (j in levels(g[,1])){
    rdd <- tryCatch(rdrobust(da[,i], df$pctfor, c = 0.5, covs = covs, covs_drop = T,
                             bwselect = j, all = 'iii'), 
    ...
    rdd.higher <- tryCatch(rdrobust(da[,i], df$pctfor, c = 0.5, h = 1.05*rdd$bws[1,],
                                    ...
                                    bwselect = j, all = 'iii'),
                           error = function(e) paste('na'))
    ce.lower.2011[i, 1] <- names(da)[i]
    ce.higher.2011[i, 1] <- names(da)[i]
    ce.lower.2011[i, j] <- ifelse(is.atomic(rdd.lower) == TRUE, 'na', 
                                  round(rdd.lower$pv[2], 3))
    ce.higher.2011[i, j] <- ifelse(is.atomic(rdd.higher) == TRUE, 'na',
                                   round(rdd.higher$pv[2], 3))
  }
}
